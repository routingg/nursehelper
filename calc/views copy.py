...
def infusion_view(request):
    ...
    if request.method == "POST":
        ...
        inputs["df"] = (request.POST.get("df") or "").strip()
        ...
        if inputs["df"]:
            try:
                df_val = float(inputs["df"])
                if df_val <= 0:
                    errors.append("드립 팩터는 양수여야 합니다.")
            except Exception:
                errors.append("드립 팩터를 숫자로 입력하세요.")

        # 계산
        if not errors and start_hm and end_hm and volume_ml is not None:
            duration_hours, end_label, crossed = _to_hours_diff(start_hm, end_hm)
            if duration_hours <= 0:
                errors.append("종료가 시작과 같거나 이전입니다. (익일 처리 실패)")
            else:
                rate_mlh = volume_ml / duration_hours  # mL/h
                ml_per_min = rate_mlh / 60.0
                ...
                # 방울(DF) 계산: 선택
                if df_val:
                    gtt_per_min = ml_per_min * df_val
                    gtt_per_sec = gtt_per_min / 60.0
                    sec_per_drop = 60.0 / gtt_per_min if gtt_per_min > 0 else None
                    result.update({
                        "gtt_per_min": gtt_per_min,
                        "gtt_per_sec": gtt_per_sec,
                        "sec_per_drop": sec_per_drop,
                    })

                # 경고(예시 기준)
                if duration_hours < 0.25:
                    result["warnings"].append("마감까지 시간이 매우 짧습니다.")
                if rate_mlh > 500:
                    result["warnings"].append("이상 고속 주입입니다. 처방/프로토콜을 재확인하세요.")
                if df_val and df_val not in df_list:
                    result["warnings"].append("비표준 드립 팩터입니다. 기기 라벨을 확인하세요.")
